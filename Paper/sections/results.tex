...
of this paper, one may visit the GitHub page for access to additional, unused 
plots. Additional relevant plots are included
...
follow-up experiments shown in \cref{app:B} \autoref{fig:gruTANH}, show
...
forecasting horizon, the differences in their degradation seem
...
changing the target causes a significant change in the behaviour of the models. 
This data suggests that utilising delta as one's forecasting target makes
...
\begin{figure}[t]
	...
	\caption{Visualisation of pruning impact. Upwards bars indicate
...
changes to each model can be seen in \autoref{fig:pruningGain}. The data shows
...
and, in particular, seems to have largely mitigated the particular
...
yesterday's price, no matter the horizon. As can be seen from
...
utilising the CatBoost architecture, be it its smaller area, less
...
if one simply chooses the correct loss function.
